				...
				>
					<div :class="{ active:item.proid==isActive }">
						<h3>
							...
							<span><strong>{{ item.type[0].price }}</strong>元/年</span>
						</h3>
						<p>
							<span>空间容量无限制</span>
							<span>带宽流量无限制</span>
...
export default {
	...
	methods: {
		...
		getdata(){
			this.$axios.get('/home/Content/prolist').then(this.thendata)
			// this.$axios.get('/api/prolist.json').then(this.thendata)
		},
		thendata(res){
			if(res.data.code == 200){
	...
	.siteTab
		...
		ul
			li
				...
				div
					min-height: 200px
					background: #fff url('/static/images/productCenter/jianzhan.png') no-repeat
