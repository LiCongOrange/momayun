		...
		<div class="content container layout1" :class="{ phoneClass: phoneToggle, width96: allEditToggle }">
			<div class="user" :style="{ opacity: opacityNum }" :class="{animated: isanimated2, fadeInLeft: isfade2}">
				<ul class="inner">
					<li v-for="(item, index) in modelInfo.userList" v-if="item.onoff">
						<span v-if="item.name != '提交'">{{ item.name }}</span>
						<!-- <input type="text" v-model="item.bindName" v-if="item.name != '留言' && item.name != '提交'"> -->
						<el-input v-model="item.bindName" v-if="item.name != '留言' && item.name != '提交'"  clearable></el-input>
						<!-- <textarea v-else-if="item.name == '留言' && item.name != '提交'" v-model="item.bindName"></textarea> -->
						<el-input type="textarea" :rows="4"  v-else-if="item.name == '留言' && item.name != '提交'" v-model="item.bindName" resize="none" clearable></el-input>
						<el-button type="warning" v-else @click="submit">{{ item.bindName }}</el-button>
			...
			>
				<div class="adduser-title">
					<h2 :style="{ opacity: opacityNum }" v-if="allEditToggle">
						<fn-editor :content="modelInfo.classTitle" :type1="7" :type2="3" :lmid="pid"></fn-editor>
					</h2>
					<h2 v-else v-html="modelInfo.classTitle"></h2>
				</div>
