				...
				<draggable v-model="modelInfo.newList" :options="{animation: 300, handle: '.list-complete-item'}" v-if="allEditToggle">
					<transition-group name="list-complete" tag="ul">
						...
						>
							...
							<div class="order" :style="{ opacity: opacityNum }">
			...
			for(let i=0; i<this.stepPages.article.length; i++) {
				if(this.stepPages.article[i].lmid == this.pid) {
					...
					for(let i=0; i<this.modelInfo.newList.length; i++) {
						if(this.modelInfo.newList[i].timer.length <= 30) {
							let obj = this.modelInfo.newList[i].timer.split(' ')
							let obj1 = obj[0].split('-')
							this.modelInfo.newList[i].timer = "<p class='ql-align-center'><p><strong>"+ obj1[2] +"</strong></p><p>"+ obj1[0] + "/" + obj1[1] +"</p></p>"
						}
					}
				}
			}
    ...
    handleScroll() {
      if(this.allEditToggle) {
		...
		addlistHandel() {
          ...
          this.$axios({
            ...
            if(res.data.code == '200') {
            	let shijian = '2018-12-04 14:01:41'
            	let shijian2 = shijian.split(' ')[0]
            	let shijian3 = shijian2.split('-')
              let obj = {
                nid: res.data.data,
                timer: "<p class='ql-align-center'><p><strong>"+ shijian3[2] +"</strong></p><p>"+ shijian3[0] + "/" + shijian3[1] +"</p></p>",
                title: '微信朋友圈推广的技巧',
        ...
        data: {
      	...
      	if(res.data.code === 200) {
      		this.modelInfo.newList.splice(index, 1)
      		this.$message({
      			type: 'success',
