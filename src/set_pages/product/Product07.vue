<template>
	<div class="grid" ref="site07" :class="[modelInfo.bgClass, phoneClass]">
		<fn-assembly
			:btnChoice="btnChoice"
			:zLayout="zLayout"
			:zLetter="zLetter"
					...
					<transition-group name="list-complete" tag="ul">
        				...
        				<i class="iconfont move" title="排序请拖动">&#xe653;</i>
							...
							<div class="img">
								...
								<div class="edit-mask-img">
									<span @click="handleEditPic(ite.pid)">编辑</span>
								</div>
								<!-- <up-img
	            		v-if="allEditToggle"
									:postImg="textImg"
	...
	methods: {
		HandleSave() {
			...
			for(let i=0; i<this.modelInfo.productList.length; i++) {
				if(this.modelInfo.productList[i].pid == this.ppid) {
					obj = {
						btnToggle: '',
						maskingToggle: '',
						nameFn: this.modelInfo.productList[i].nameFn,
						past: this.modelInfo.productList[i].past,
						picurl: this.textImg,
						pid: this.modelInfo.productList[i].pid,
						title: this.modelInfo.productList[i].title
					}
					currentIndex = i
				}
			}
			this.$axios({
				...
				if(res.data.code == '200') {
					...
					this.modelInfo.productList[currentIndex].picurl = this.textImg
      ...
      if(this.allEditToggle) {
		...
		addList() {
			let obj = {
				...
				nameFn: "",
				past: "",
				picurl: "/static/images/product/shows/pro54.jpg"
			}
