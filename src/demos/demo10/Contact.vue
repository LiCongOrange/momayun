<template>
	...
	>
		...
		<div 
			class="content container layout1" 
			:class="{ phoneClass: phoneToggle, width96: allEditToggle}"
		>
			<div class="user" :style="{ opacity: opacityNum }" :class="{animated: isanimated2, slideInLeft: isfade2}">
				<ul class="inner">
				...
				<div class="edit-overlay" v-if="allEditToggle">
					<div v-if="userToggle" class="zhezhao">
						<div class="mask">
				...
				<div class="add-overlay" v-if="allEditToggle">
					...
					<div class="add-overlay-main" v-else>
						...
						<ul>
							<li v-for="(item, index) in modelInfo.classList">
								<span>{{item.name}} : </span>
								<textarea v-model="item.val" v-if="item.val == '地址'"></textarea>
...
export default {
    ...
    handleScroll() {
      if(this.allEditToggle) {
      	...
      	let htmlT = this.$refs.contact1.getBoundingClientRect().top
	      if(htmlT <= 560) {
